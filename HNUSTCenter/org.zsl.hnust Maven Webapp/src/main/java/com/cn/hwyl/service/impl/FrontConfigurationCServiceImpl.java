...
public class FrontConfigurationCServiceImpl implements IFrontConfigurationService {

	private static Logger logger = Logger.getLogger(FrontConfigurationCServiceImpl.class);
	@Resource 
	...
	@Override
	public int insertSelective(FrontConfiguration record) {
		return frontConfigurationDao.insertSelective(record);
	}
	...
	@Override
	public int updateByPrimaryKeySelective(FrontConfiguration record) {
		return frontConfigurationDao.updateByPrimaryKeySelective(record);
	}
	...
	public Result insertOrgAndsyncFranc(FrontConfiguration record) {
		...
		try {
			...
			logger.info("中心端插入服务器配置信息successful!!"+record.getcOrgid());
	...
	private Result createAdminRoleAndSynchronousFront(FrontConfiguration frontConfiguration,Organization organization,HospitalServiceCenter hospitalServiceCenter,Channel channel) {
		...
		if (frontConfiguration != null) {
			...
			logger.info("中心端同步组织到前置端 begin ！！"+organization.getOrgId());
			resultFront = HWYLWebService.getWSDL(wsdlUrl, nameSpace, method, gson.toJson(head), gson.toJson(map));
			logger.info("中心端同步组织到前置端 successful ！！"+organization.getOrgId());
		}
		...
		if ("0".equals(result.getResultCode())) {
			logger.error("中心端同步组织到前置端失败!!" + result.getErrorInfo()+organization.getOrgId());
			result.setResultCode("0");
			result.setErrorInfo("中心端同步组织到前置端失败!!");
			throw new RuntimeException();
			}
		// 2,在中心端创建
		int roleResult = roleDao.insert(role);
		if (roleResult > 0) {
			logger.info("中心端创建管理员角色successful！！"+organization.getOrgId());
			result.setResultCode("1");
			result.setErrorInfo("中心端创建管理员角色successful！！");
		}
		result.setResultCode("1");
		return result;
	}
