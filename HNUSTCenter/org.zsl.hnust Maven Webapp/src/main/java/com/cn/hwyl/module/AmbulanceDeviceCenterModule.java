...
public class AmbulanceDeviceCenterModule {
	private static Logger logger = Logger.getLogger(FirstAidCenterModule.class);
	...
	public static String ambulanceDeviceAll(HeadRequest head, BodyRequest body) {
		...
		if ("AmbulanceDevice_DeviceUpdate".equals(head.getService())) {
			if (body.getDeviceUpdateBody() == null || "".equals(body.getDeviceUpdateBody())) {
				result.setResultCode("0");
				result.setErrorInfo("传参为空");
				return gson.toJson(result);
			}
		...
		if("AmbulanceDevice_DeviceUpdate_select".equals(head.getService())){
			if (body.getDeviceUpdatSelectBody() == null || "".equals(body.getDeviceUpdatSelectBody())) {
				result.setResultCode("0");
				result.setErrorInfo("传参为空");
				return gson.toJson(result);
			}
