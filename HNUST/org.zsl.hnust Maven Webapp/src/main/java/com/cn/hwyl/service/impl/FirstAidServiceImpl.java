...
public class FirstAidServiceImpl implements IFirstAidService{
	...
	@Override
	public String selectFirstAidIDBycCardNumber(String cCardNumber) {
		return firstAidDao.selectFirstAidIDBycCardNumber(cCardNumber);
	}
	...
	public Result saveOrUpdateBody(HeadRequest head, BodyRequest body) {
		...
		try {
			...
			if (body.getFirstAidEditBody().getPatients() != null) {
				...
				// 3.1修改患者信息的时候先同步到云端
				head.setService("UpdateWardUserInfo");
				
				WardshipViewObject wardUserInfo= CommonUtil.getWardshipViewObjectByPatients(body.getFirstAidEditBody().getPatients());
				BodyRequest bodyParam = new BodyRequest();
				bodyParam.setWardUserInfo(wardUserInfo);
				String resultyun = HWYLWebService.getWSDL(WSDLProperties.getYunWsdl(), WSDLProperties.getYunNameSpace(),
						"callService", gson.toJson(head), gson.toJson(bodyParam));
			...
			//5保存更改记录
			if(body.getFirstAidEditBody().getFirstaidCollection() !=null){
				firstaidCollectionDao.insert(body.getFirstAidEditBody().getFirstaidCollection());
			}
		} catch (Exception e) {
			result.setResultCode("0");
			logger.error("程序异常:" + e);
