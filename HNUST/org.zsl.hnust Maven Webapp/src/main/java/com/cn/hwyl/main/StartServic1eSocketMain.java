...
import com.cn.hwyl.socket.ChatServerByCardNumber;

/**
 * Trigger to consume data from Nucleon Event Bus
 */
//@Component("processor")
public class StartServic1eSocketMain extends HttpServlet {

	private static final long serialVersionUID = -9045451275234606838L;

	private static final Logger logger = Logger.getLogger(StartServic1eSocketMain.class);
	...
	private void start() {
		try {
			ChatServerByCardNumber chatServerByCardNumber = new ChatServerByCardNumber();
			chatServerByCardNumber.startWork();
		} catch (Exception e) {
